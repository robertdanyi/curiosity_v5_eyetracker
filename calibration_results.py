...
"""
TOBII DATA STRUCTURES:
calibration_result: a Calibration_Result object, represents the result of the calculated calibration :)
    fields: status, calibration_points
calibration_result.calibration_points: a tuple of CalibrationPoint objects.
A CalibrationPoint object represents the calibration point and its collected calibration samples.
    fields: position_on_display_area, calibration_samples
A CalibrationSample object represents the data collected for a calibration sample
    fields: left_eye, right_eye (objects representing the calibration data collected for an eye)
"""

"""NOTES:
   1. All original self.points_to_calibrate are needed to draw all calibration circles even if some points are not in calibration_result.
   2. If points < 3, calibration is unsuccesful at Tobii's side, and no calibration_result data is generated
   3. psychopy pixel screen coordinates are: top left = (-dispsize[0]/2, dispsize[1]/2), center = (0,0))
"""
